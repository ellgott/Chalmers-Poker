...
class Check():
    ...
    def Straight(self, Announce=True):
    ...
    def FullHouse(self, Announce=True):
    ...
    def StraightFlush(self, Announce=True):
        ...
        for i in range(len(players)):
            ...
            if straight != 0:
                ...
                if flush != 0:
                    ...
                    if Announce:
                        print(f'Player {i+1:.0f} has Straight Flush in {flush_color} up to {np.max(latter)}!')

            players_ranks[i] = rank_list

        return players_ranks
    
    def Highest(self, Announce=True):
    ...
    def Total(self, Announce=True):
        
        players_ranks = self.Pairs(Announce)
        players_ranks += self.Three(Announce)
        players_ranks += self.FullHouse(Announce)
        players_ranks += self.Flush(Announce)
        players_ranks += self.Straight(Announce)
        players_ranks += self.Four(Announce)
        players_ranks += self.StraightFlush(Announce)
        players_ranks += self.Highest(Announce)
